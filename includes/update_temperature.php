...
error_log('update_temperature.php was called - IP: ' . $_SERVER['REMOTE_ADDR'] . ' - Data: ' . $requestData);
...
try {
    ...
    if (!isset($input['temperature_id'])) {
        throw new Exception('ID da temperatura não informado');
    }

    $temperatureId = (int)$input['temperature_id'];
    ...
    error_log('Erro em update_temperature.php: ' . $e->getMessage() . ' - IP: ' . $_SERVER['REMOTE_ADDR']);
